...
class Elasticsearch {
	 ...
	 * @return [type] [description]
	 */
	private function buildGeoSort()
	{
		//是否需要根据 geo 来排序
		if ($this->geoParam['needSort'] === TRUE)
		{
			$sort               = isset($this->geoParam['sort']) ? $this->geoParam['sort'] : 'asc';
			$this->data['sort'] = array(
				array(
					'_geo_distance' => array(
						'order'                 => $sort,
						"unit"                  => $this->geoParam['unit'],
						$this->geoParam['attr'] => array(
							'lon' => $this->geoParam['lon'],
							'lat' => $this->geoParam['lat'],
						),
	...
	/**
	 * 动态计算排序, 需要手动传入 script 和参数
	 * @return [type] [description]
	 */
	private function scriptScoreSrot()
	{
		$sortArr            = array(
			'_script' => array(
				'type'   => 'number',
