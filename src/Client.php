...
class Client extends DSLBuilder {
	...
	public function select($source)
	{
		if ( ! is_array($source))
		{
			throw new \Exception('source field must be an array');
		}
	...
	public function update($index, $type, $id, $data, $routing = '')
	{
		...
		{
			throw new \Exception('in update , you must set index type id data');
		}
		$this->buildDsl();
		$params = array(
			'index' => $index,
	...
	public function insert($index, $type, $id, $data, $routing = '')
	{
		if (empty($index) || empty($type) || empty($id) || empty($data))
		{
			throw new \Exception('in create , you must set index type id data');
		}
		$this->buildDsl();
		$params = array(
			'index' => $index,
	...
	public function delete($index, $type, $id)
	{
		...
		{
			throw new \Exception('in delete , you must set index type id');
		}
		$this->buildDsl();
		$params = array(
			'index' => $index,
